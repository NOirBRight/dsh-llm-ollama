...
import { OLLAMA_PROVIDER, OLLAMA_SETTINGS_NAMESPACE } from '../src/client-contract.ts'
...
describe('settings mutate validation', () => {
  it('rejects duplicate catalog model ids and leaves saved and runtime state unchanged', async () => {
    ...
    const before = ctx.settings.describe().find(item => item.ns === OLLAMA_SETTINGS_NAMESPACE)
    expect(before).toBeDefined()
    const ids = (await ctx.llm.listModels(OLLAMA_PROVIDER)).map(model => model.id)
    expect(ids).toContain('kept-model')

    await expect(ctx.settings.mutate(OLLAMA_SETTINGS_NAMESPACE, [
      { op: 'set', path: ['models'], value: [{ id: 'kept-model' }, { id: 'kept-model' }] },
    ])).rejects.toThrow(/duplicate/)

    const after = ctx.settings.describe().find(item => item.ns === OLLAMA_SETTINGS_NAMESPACE)
    expect(after?.revision).toBe(before?.revision)
    expect(after?.value).toEqual(before?.value)
    expect(settings.writes).toHaveLength(0)
    expect((await ctx.llm.listModels(OLLAMA_PROVIDER)).map(model => model.id)).toEqual(ids)
  })
})
