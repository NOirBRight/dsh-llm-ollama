...
这是 Harness LLM 能力的 Ollama Cloud 原生聊天适配器：通过直接 `fetch` 和 NDJSON（逐行 JSON）把 Ollama 原生 `/api/chat` 线协议转换为 `StreamChunk` 协议。模型发现使用公共端点的 `/api/tags`，按原生 tag id 去重，再通过 `/api/show` 获取 OpenAI 兼容 `/v1/models` 列表不提供的上下文窗口与能力（视觉、推理、工具调用）。同一个 Host 插件还把 Ollama 的 `/api/web_search` 与 `/api/web_fetch` 以 `ollama-cloud` 为 id 注册进 Harness 的 web 能力 seam。
...
- **发现**使用 `/api/tags` 和 `/api/show`。原生响应会返回 `model_info.*.context_length` 与 `capabilities`（视觉、推理、工具调用）；OpenAI 兼容 `/v1/models` 只返回模型 id。
...
插件为 `llm-ollama` 设置命名空间注册模型发现 handler。配置界面的“获取可用模型”先调用 `GET /api/tags`；按原生 tag id 去重。随后逐个调用 `POST /api/show` 提取：
...
幂等的 tags 请求会在一次传输失败后重试；持续失败时只报告不含凭据的网络详情。候选元数据显示在 frame overlay 选择器中；应用选择会替换草稿模型目录，只有保存到设置分节的模型才会由 route 公开。
