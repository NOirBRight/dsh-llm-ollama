		...
		*/
		...
		function apply(ctx) {
			...
			const saveConfiguration = async (settings) => {
				...
				if (current.baseURL === settings.baseURL && JSON.stringify(current.models) === JSON.stringify(settings.models)) return {
					settings: current,
				...
				if (!await settingsForm.mutate([{
					...
					if (latest.revision !== void 0 && latest.revision !== snapshot.revision) throw new Error(`settings namespace "${OLLAMA_SETTINGS_NAMESPACE}" changed since it was read (expected revision ${snapshot.revision}, now ${latest.revision})`);
