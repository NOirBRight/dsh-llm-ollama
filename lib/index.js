...
*/
async function* translate(chunks, options = {}) {
	...
	for await (const chunk of chunks) {
		...
		for (const call of message.tool_calls ?? []) {
			...
			const callId = typeof call.id === "string" && call.id.length > 0 ? call.id : `${callIdPrefix}-${ordinal}`;
