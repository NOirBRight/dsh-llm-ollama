...
function resolveThink(options, defaults) {
	...
	if (effort === "off") {
		if (!canDisable) throw new LlmError("Ollama model does not support disabling thinking", "UNSUPPORTED_REASONING_EFFORT");
		return { think: false };
	}
	return { think: effort };
}
...
*/
function stripHtmlCloudSuffix(id) {
...
*/
function extractCloudModelIds(html) {
	const ids = /* @__PURE__ */ new Set();
	for (const match of html.matchAll(MODEL_CARD_ANCHOR)) {
		const attributes = match[1];
		const card = match[2];
		if (attributes === void 0 || card === void 0 || !CLOUD_BADGE.test(card)) continue;
		...
		const encodedId = path.slice(9).replace(/^\/+|\/+$/g, "");
...
async function fetchCloudSearch(signal) {
	try {
		...
		if (signal?.aborted) throw new LlmError("model discovery aborted by caller", "ABORTED", { cause: error });
		throw new LlmError("could not reach https://ollama.com/search?c=cloud" + networkDetail(error), "DISCOVERY_FAILED", { cause: error });
	}
}
...
async function discoverCloudModelIds(signal) {
	let failure;
	for (let attempt = 0; attempt < CLOUD_SEARCH_ATTEMPTS; attempt += 1) try {
		const response = await fetchCloudSearch(signal);
		if (!response.ok) throw new LlmError("https://ollama.com/search?c=cloud answered " + response.status, "DISCOVERY_FAILED");
		const ids = extractCloudModelIds(await readBounded(response, PUBLIC_CLOUD_SEARCH_URL));
		if (ids.length === 0) throw new LlmError("https://ollama.com/search?c=cloud did not contain cloud model cards", "DISCOVERY_FAILED");
		return ids;
	} catch (error) {
		if (signal?.aborted) throw new LlmError("model discovery aborted by caller", "ABORTED", { cause: error });
		failure = error;
	...
	throw new LlmError("could not read https://ollama.com/search?c=cloud" + networkDetail(failure), "DISCOVERY_FAILED", { cause: failure });
}
/**
...
*/
function mergeCloudModels(tags, cloudIds) {
	const merged = [];
...
async function discoverModels(request, storedApiKey) {
	...
	const cloudIds = baseURL === PUBLIC_BASE_URL ? await discoverCloudModelIds(request.signal) : [];
	const mergedTags = mergeCloudModels(tagsBody.models, cloudIds);
	const models = new Array(mergedTags.length);
	let cursor = 0;
	const worker = async () => {
		for (;;) {
			const index = cursor;
	...
	await Promise.all(Array.from({ length: Math.min(SHOW_CONCURRENCY, mergedTags.length) }, worker));
...
async function postJson(options, suffix, payload, signal) {
	...
	try {
		...
		if (signal?.aborted) throw new WebError("ollama-cloud web request aborted by caller", "ABORTED", { cause: error });
...
//#endregion
export { Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_STREAM_IDLE_TIMEOUT_MS, OLLAMA_DISCOVER_ENDPOINT, OLLAMA_PROVIDER, OLLAMA_PUBLIC_BASE_URL, OLLAMA_RPC_CHANNEL, OLLAMA_SAVE_ENDPOINT, OLLAMA_SETTINGS_NAMESPACE, OLLAMA_WEB_PROVIDER_ID, OllamaAdapter, OllamaWebFetchProvider, OllamaWebSearchProvider, PUBLIC_BASE_URL, apply, decodeOllamaCatalogModel, decodeOllamaDiscoveryRequest, decodeOllamaDiscoveryResult, decodeOllamaSaveRequest, decodeOllamaSaveResult, decodeOllamaSettings, discoverModels, extractCapabilities, extractContextWindow, inject, name, resolveAdapterOptions };
