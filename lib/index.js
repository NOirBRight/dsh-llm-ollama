...
function decodeOllamaSaveResult(value) {
	if (!isRecord$2(value) || !Number.isSafeInteger(value["revision"])) return void 0;
...
function decodeOllamaSettingsReadResult(value) {
	...
	if (settings === void 0 || !isRecord$2(credential) || typeof credential["configured"] !== "boolean" || typeof credential["writable"] !== "boolean") return void 0;
	const revision = value["revision"];
...
function decodeOllamaCredentialRef(value) {
	if (!isRecord$2(value) || typeof value["ref"] !== "string" || value["ref"].length === 0) return void 0;
	return value["ref"];
}
function decodeOllamaCredentialSetRequest(value) {
	if (!isRecord$2(value) || typeof value["ref"] !== "string" || value["ref"].length === 0 || typeof value["value"] !== "string" || value["value"].length === 0) return void 0;
	return {
		ref: value["ref"],
		value: value["value"]
	};
...
function decodeOllamaCredentialStatus(value) {
	if (!isRecord$2(value) || typeof value["configured"] !== "boolean" || typeof value["writable"] !== "boolean") return void 0;
	return {
...
function apply(ctx, config) {
	...
	if (Object.hasOwn(config, "remoteManagement")) throw new Error("llm-ollama: remoteManagement is not supported by the Alpha.4 Connection service");
	let current = () => config;
	let lastRaw;
	let lastGood;
	const options = () => {
	...
	ctx.inject(["connection", "webServer"], (connectionCtx) => {
		...
		const handler = async (endpoint, payload, signal) => {
			if (endpoint === "settings/read") {
				...
				if (descriptor === void 0 || decoded === void 0 || credentials === void 0) return settingsFailure("Ollama Cloud settings are unavailable");
			...
			if (endpoint === "credential/status") {
				...
				const decoded = decodeOllamaSettings(descriptor?.value);
				if (ref === void 0 || credentials === void 0 || decoded === void 0 || ref !== decoded.apiKeyEnv) return settingsFailure("invalid Ollama Cloud credential request");
			...
			if (endpoint === "credential/set") {
				...
				const descriptor = ctx.get("settings")?.describe().find((item) => item.ns === NS);
				const decoded = decodeOllamaSettings(descriptor?.value);
				if (request === void 0 || credentials === void 0 || decoded === void 0 || request.ref !== decoded.apiKeyEnv) return settingsFailure("invalid Ollama Cloud credential request");
				try {
...
//#endregion
export { Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_STREAM_IDLE_TIMEOUT_MS, DEFAULT_USAGE_REQUEST_TIMEOUT_MS, DEFAULT_WEB_REQUEST_TIMEOUT_MS, OLLAMA_CREDENTIAL_SET_ENDPOINT, OLLAMA_CREDENTIAL_STATUS_ENDPOINT, OLLAMA_DISCOVER_ENDPOINT, OLLAMA_PROVIDER, OLLAMA_PUBLIC_BASE_URL, OLLAMA_RPC_CHANNEL, OLLAMA_SAVE_ENDPOINT, OLLAMA_SETTINGS_NAMESPACE, OLLAMA_SETTINGS_READ_ENDPOINT, OLLAMA_USAGE_ENDPOINT, OLLAMA_USAGE_FAILED, OLLAMA_USAGE_UNSUPPORTED, OLLAMA_WEB_PROVIDER_ID, OllamaAdapter, OllamaWebFetchProvider, OllamaWebSearchProvider, PUBLIC_BASE_URL, apply, decodeOllamaCatalogModel, decodeOllamaCredentialRef, decodeOllamaCredentialSetRequest, decodeOllamaCredentialStatus, decodeOllamaDiscoveryRequest, decodeOllamaDiscoveryResult, decodeOllamaSaveRequest, decodeOllamaSaveResult, decodeOllamaSettings, decodeOllamaSettingsReadResult, decodeOllamaUsageReply, discoverModels, extractCapabilities, extractContextWindow, inject, name, parseOllamaUsage, readOllamaUsage, resolveAdapterOptions };
