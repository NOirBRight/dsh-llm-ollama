...
import { INVALID_CREDENTIAL_CODE, LlmAdapter, LlmError, RetryPolicySchema, assertUsableApiKey, attributionHeaders, isHarnessError, normalizeApiKey, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
function parseOllamaPickerId(id) {
	let rest = id;
	let fast = false;
...
function decodeOllamaSettings(value) {
	if (!isRecord$2(value) || typeof value["baseURL"] !== "string" || value["baseURL"].length === 0) return void 0;
...
function decodeOllamaDiscoveryResult(value) {
	if (!isRecord$2(value) || !Array.isArray(value["models"])) return void 0;
	const models = [];
	for (const model of value["models"]) {
		const decoded = decodeOllamaCatalogModel(model);
		if (decoded === void 0) return void 0;
...
function decodeOllamaUsageWindow(value) {
	...
	if (modelsValue !== void 0) {
		...
		for (const entry of modelsValue) {
			if (!isRecord$2(entry) || typeof entry["name"] !== "string" || entry["name"].length === 0) return void 0;
...
*/
function decodeOllamaSettingsValidationRequest(value) {
	if (!isRecord$2(value) || typeof value["baseURL"] !== "string" || value["baseURL"].length === 0) return void 0;
	try {
...
function decodeOllamaCredentialStatus(value) {
	if (!isRecord$2(value) || typeof value["configured"] !== "boolean" || typeof value["writable"] !== "boolean") return;
	return {
		configured: value["configured"],
		writable: value["writable"]
	};
}
function decodeOllamaCredentialSetRequest(value) {
	if (!isRecord$2(value) || typeof value["value"] !== "string" || value["value"].length === 0) return void 0;
...
async function discoverModels(request, storedApiKey, signal) {
	...
	const worker = async () => {
		for (;;) {
			...
			if (tag !== void 0) models[index] = await discoverTaggedModel(tag, baseURL, apiKey, signal);
		}
	};
...
function withOllamaWire(streamFn) {
	return (model, context, options) => {
		const parsed = parseOllamaPickerId(model.id);
		return streamFn(parsed.wireId === model.id ? model : {
			...model,
			id: parsed.wireId
		}, context, options);
	};
...
*/
function classifyOllamaTransientError(chunk) {
	if (chunk.type !== "finish" || chunk.reason.kind !== "error" || chunk.reason.failure.code !== "PI_AI_ERROR") return chunk;
	if (!/model failed to generate a response|error was encountered while running the model|cloud model cannot be reached|server is overloaded/iu.test(chunk.reason.failure.message)) return chunk;
	return {
		...chunk,
		reason: {
			...chunk.reason,
...
*/
function narrowOllamaEscalationSchemas(options) {
	...
	const tools = options.tools.map((tool) => {
		...
		const properties = isRecord$1(parameters.properties) ? parameters.properties : void 0;
		const permission = properties === void 0 || !isRecord$1(properties.sandbox_permissions) ? void 0 : properties.sandbox_permissions;
...
var OllamaAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		for await (const chunk of this.current().stream(narrowOllamaEscalationSchemas(options))) yield classifyOllamaTransientError(chunk);
	}
	/**
	...
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: async function* (options) {
				for await (const chunk of inner.stream(narrowOllamaEscalationSchemas(options))) yield classifyOllamaTransientError(chunk);
			}
		};
	}
	/**
...
function parseResetAt(record, now) {
	const direct = isoInstant(record["resets_at"] ?? record["reset_at"] ?? record["reset"] ?? record["resetsAt"]);
...
*/
function parseOllamaUsage(value, url, now = Date.now()) {
	const limits = isRecord(value) ? value.limits : void 0;
	const windows = {};
	if (isRecord(limits)) for (const key of USAGE_WINDOW_KEYS) {
	...
	if (Object.keys(windows).length === 0) {
		...
		throw new LlmError(`${url} returned a malformed usage response (limits keys: ${observed.length === 0 ? "none" : observed.join(", ")})`, OLLAMA_USAGE_FAILED);
	}
	return {
...
*/
function settingsFailure(message, code = "internal") {
	return {
		ok: false,
		error: {
			code,
...
function usageFailure(error) {
	...
	if (error.code === "OLLAMA_USAGE_UNSUPPORTED") return {
		ok: true,
		value: { status: "unsupported" }
	};
	return settingsFailure(error.message.length > 0 ? error.message : "Ollama Cloud usage read failed", error.code);
}
function apply(ctx, config) {
	...
	const options = () => {
		...
		if (baseURL === lastBaseURL && models === lastModels && lastGood !== void 0) return lastGood;
	...
	ctx.inject(["connection", "webServer"], (connectionCtx) => {
		const usageApiKey = async () => {
			try {
				return await storedApiKey();
			} catch (error) {
				if (!isHarnessError(error) || !CREDENTIAL_FAILURE_CODES.includes(error.code)) throw error;
				throw new LlmError(error.message.length > 0 ? error.message : "Ollama Cloud credential lookup failed", INVALID_CREDENTIAL_CODE, { cause: error });
			}
		};
		const handler = async (endpoint, payload, signal) => {
			if (endpoint === "credential/status") {
				if (payload !== void 0 && (typeof payload !== "object" || payload === null || Array.isArray(payload) || Object.keys(payload).length > 0)) return settingsFailure("invalid Ollama Cloud credential request");
				const credentials = ctx.get("credentials");
				if (credentials === void 0) return settingsFailure("Ollama Cloud credentials are unavailable");
			...
			if (endpoint === "credential/set") {
				...
				const credentials = ctx.get("credentials");
				if (request === void 0 || credentials === void 0) return settingsFailure("invalid Ollama Cloud credential request");
				try {
					const ref = options().apiKeyEnv;
			...
			if (endpoint === "models/discover") {
				...
				try {
					return {
						ok: true,
						value: { models: await discoverModels(request, storedApiKey, signal) }
					};
				} catch (error) {
			...
			if (endpoint === "settings/validate") {
				...
				if (descriptor === void 0) return settingsFailure("Ollama Cloud settings are unavailable");
				if (descriptor.revision !== request.expectedRevision) return settingsFailure(`settings namespace "${NS}" changed since it was read (expected revision ${request.expectedRevision}, now ${descriptor.revision})`, "SETTINGS_CONFLICT");
				try {
					const { baseURL: _baseURL, models: _models, ...rest } = config;
					...
					return settingsFailure(error instanceof Error && error.message.length > 0 ? error.message : "invalid Ollama Cloud settings request");
				}
			}
		...
		connectionCtx.effect(() => connectionCtx.connection.fetch.register({
			...
			fetch: async (request) => {
				...
				if (!parsed.success || parsed.data.method !== "plugin-rpc/ollama-cloud") return new Response("invalid plugin RPC request", { status: 400 });
				const wrapper = parsed.data.payload;
				if (typeof wrapper !== "object" || wrapper === null || Array.isArray(wrapper)) return new Response("invalid plugin RPC payload", { status: 400 });
				const envelope = wrapper;
				if (typeof envelope["endpoint"] !== "string") return new Response("invalid plugin RPC payload", { status: 400 });
				let result;
				try {
					result = await handler(envelope["endpoint"], envelope["payload"], request.signal, connectionCtx.connection.operator);
				} catch {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.effect(() => settingsCtx.settings.configure({ auto: false }, ctx.fiber), "llm-ollama: settings page policy");
	});
}
//#endregion
export { Config, DEFAULT_API_KEY_ENV, DEFAULT_CONTEXT_WINDOW, DEFAULT_STREAM_IDLE_TIMEOUT_MS, DEFAULT_USAGE_REQUEST_TIMEOUT_MS, DEFAULT_WEB_REQUEST_TIMEOUT_MS, OLLAMA_CREDENTIAL_SET_ENDPOINT, OLLAMA_CREDENTIAL_STATUS_ENDPOINT, OLLAMA_DISCOVER_ENDPOINT, OLLAMA_PROVIDER, OLLAMA_PUBLIC_BASE_URL, OLLAMA_RPC_METHOD, OLLAMA_SETTINGS_NAMESPACE, OLLAMA_SETTINGS_VALIDATE_ENDPOINT, OLLAMA_USAGE_ENDPOINT, OLLAMA_USAGE_FAILED, OLLAMA_USAGE_UNSUPPORTED, OLLAMA_WEB_PROVIDER_ID, OllamaAdapter, OllamaWebFetchProvider, OllamaWebSearchProvider, PUBLIC_BASE_URL, apply, decodeOllamaCatalogModel, decodeOllamaCredentialSetRequest, decodeOllamaCredentialStatus, decodeOllamaDiscoveryRequest, decodeOllamaDiscoveryResult, decodeOllamaSettings, decodeOllamaSettingsValidationRequest, decodeOllamaUsageReply, discoverModels, extractCapabilities, extractContextWindow, inject, name, parseOllamaUsage, readOllamaUsage, resolveAdapterOptions };
