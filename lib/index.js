	...
	ctx.inject(["connection"], (connectionCtx) => {
		const usageApiKey = async () => {
			try {
				...
				throw new LlmError(error instanceof Error && error.message.length > 0 ? error.message : "Ollama Cloud credential lookup failed", INVALID_CREDENTIAL_CODE, { cause: error });
			}
		};
