...
function decodeOllamaSettingsReadResult(value) {
	...
	if (settings === void 0 || !isRecord$1(credential) || typeof credential["configured"] !== "boolean" || typeof credential["writable"] !== "boolean") return void 0;
	const revision = value["revision"];
...
function decodeOllamaCredentialRef(value) {
	if (!isRecord$1(value) || typeof value["ref"] !== "string" || value["ref"].length === 0) return void 0;
	return value["ref"];
}
function decodeOllamaCredentialSetRequest(value) {
	if (!isRecord$1(value) || typeof value["ref"] !== "string" || value["ref"].length === 0 || typeof value["value"] !== "string" || value["value"].length === 0) return void 0;
	return {
		ref: value["ref"],
		value: value["value"]
	};
...
function decodeOllamaCredentialStatus(value) {
	if (!isRecord$1(value) || typeof value["configured"] !== "boolean" || typeof value["writable"] !== "boolean") return void 0;
	return {
...
var OllamaAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		for await (const chunk of this.current().stream(options)) yield classifyOllamaTransientError(chunk);
	}
	...
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: async function* (options) {
				for await (const chunk of inner.stream(options)) yield classifyOllamaTransientError(chunk);
			}
		};
	}
};
