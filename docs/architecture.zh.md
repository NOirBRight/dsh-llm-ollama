...
`dsh-llm-ollama` 适配器只实现原生 `/api/chat` 协议。它遵循 `dsh-llm-deepseek` 的直接 fetch 模板（分拆 wire-types / serialize / NDJSON-parse / translate / adapter 模块），通过 `/api/tags` 与 `/api/show` 发现模型；默认公共端点还从 `/search?c=cloud` 补充 cloud 模型卡片，移除 HTML 中的 `-cloud` 后缀，按 id 去重且不还原该后缀。
...
- **发现丰富度**：`/api/tags` + `/api/show` 发现返回 OpenAI 兼容列表无法提供的上下文窗口和能力标志。默认公共端点会从 `/search?c=cloud` 补充 cloud 模型卡片；HTML 中的 `-cloud` 后缀会被移除且不还原，重复 id 会丢弃。client 会在 RPC 完成前打开选择器；Host 最多并发丰富六个合并后的模型，保留 tag 顺序并追加只在 cloud 列表中的模型。包的 loopback RPC 为自己的 client 卡片保留 Ollama 专用的 vision、thinking 和 tools 标志。`/api/show` 不会标识可接受的 thinking 等级。
