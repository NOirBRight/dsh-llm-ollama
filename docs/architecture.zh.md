...
- **发现丰富度**：`/api/tags` + `/api/show` 发现返回 OpenAI 兼容列表无法提供的上下文窗口和能力标志。标准 LLM 发现响应携带可移植的容量字段；包的 loopback 配置 RPC 为自己的 client 卡片保留 Ollama 专用的 vision、thinking 和 tools 标志。`/api/show` 不会标识可接受的 thinking 等级。
